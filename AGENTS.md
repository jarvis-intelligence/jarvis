...
Contributor guide for **codeintel** — a local-first code intelligence MCP server
(SCIP navigation + Zoekt search). The distribution is published as
`codeintel-navigation-mcp`; the import package, both CLIs, and the MCP server
are all `codeintel`.

## Project Structure & Module Organization

`src/codeintel/` — one module per concern:

- `index_cli.py` — the `codeintel` CLI (subcommands `index`/`list`/`status`/`reindex`/`forget`/`watch`) and the indexing pipeline: detect language → run the matching SCIP indexer → `scip expt-convert` → populate graph → `zoekt-index` → atomic pointer-swap publish.
...
- `graph.py` + `registry.py` — two SQLite stores over `~/.codeintel/registry.db`. `graph.py` is the package dependency graph (`packages`/`edges`) driving `blastRadius` (2-hop BFS); `registry.py` is the repo registry (`repos` table).
...
- `pyproject.toml` — distribution `codeintel-navigation-mcp`, `uv_build` backend, `[tool.uv.build-backend] module-name = "codeintel"`.
- `docs/`, `evals/`, `plans/` — architecture, code standards, roadmap, eval harness, and design plans.
- `plugin/` — Claude Code plugin (`.claude-plugin/plugin.json` manifest, `.mcp.json`, and skills under `skills/codeintel-setup`/`codeintel-use`/`codeintel-issues`).
- `.claude/skills/codeintel-release` — maintainer-only release skill (distinct from `plugin/`, which ships to end users).
...
- `uv run codeintel index /path/to/repo [--slug name] [--language name] [--scheme name] [--semantic-include path] [--search-only]` — index a repo.
- `uv run codeintel list` / `status <slug>` / `reindex <slug>` / `forget <slug>` — registry operations.
- `uv run codeintel watch /path/to/repo [--debounce N] [--scheme name] [--language name] [--semantic-include path]` — foreground auto-reindex on file changes.
- `uv run codeintel-server` — MCP stdio entry point.
- `uv run python scripts/check_versions.py` — verify version sources agree.

Optional extras (`watch`, `semantic`) must be installed via `--extra <name>`; their imports are always deferred inside the functions that need them, so a base install can still import every module in `src/codeintel/`. Required on `PATH` for anything beyond unit tests: one language indexer per repo (`scip-typescript` / `scip-python` / `scip-java` / `scip-swift`), `scip` (for `scip expt-convert`), and `zoekt-index` / `zoekt-webserver`.

## MCP Tools

`server.py` registers nine tools. Each nav tool takes `repo` (the slug from `codeintel index`) plus a tool-specific `symbol` or `path`; all tools catch exceptions broadly and return `{"error": "..."}` rather than raising, to keep the stdio server alive:
...
- Environment variable overrides are `CODEINTEL_`-prefixed: `CODEINTEL_DATA_DIR`, `CODEINTEL_ZOEKT_BIN`, `CODEINTEL_EMBEDDING_MODEL`, `CODEINTEL_EMBEDDING_BATCH_SIZE`, `CODEINTEL_EMBEDDING_QUERY_PREFIX`, `CODEINTEL_EMBEDDING_DOC_PREFIX`.
