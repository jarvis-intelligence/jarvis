...
* No authorization filtering anywhere — codeintel has no multi-tenancy, so
...
from codeintel.query import FreshnessSnapshot
from codeintel.scip_decoder import SymbolRoles, parse_symbol_package
...
def blast_radius(store: GraphStore, repo: str, name: str) -> BlastRadiusResult:
    ...
    if seed is None:
        ...
        hint = f"known packages for {repo!r}: {known}" if known else f"{repo!r} has no packages registered at all — has `codeintel index` been run for it?"
