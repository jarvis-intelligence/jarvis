...
| `uv.lock` | **never hand-edit.** Run `uv lock` — it updates this package's own self-referential version entry (`Updated codeintel-navigation-mcp vX.Y.Z-1 -> vX.Y.Z` in its output) and re-resolves nothing else changes if no deps moved |
...
grep -qF "mcp-name: io.github.phuongddx/codeintel" README.md && echo "marker present"
```
...
See [CHANGELOG.md](https://github.com/phuongddx/codeintel/blob/main/CHANGELOG.md).
EOF
)"
```
...
```bash
gh run list --repo phuongddx/codeintel --workflow=publish-pypi.yml --limit 1
gh run watch <run-id> --repo phuongddx/codeintel --exit-status   # or gh run view <run-id> if it already finished
gh run list --repo phuongddx/codeintel --workflow=publish-mcp-registry.yml --limit 1
```
