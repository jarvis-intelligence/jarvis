...
See [`plans/0724-2316-jarvis-mcp-implementation/`](../plans/0724-2316-jarvis-mcp-implementation/) for the detailed implementation plan and phase documentation.
...
| **4** | blastRadius (package graph) + jarvis watch (auto-reindex) | ✓ July 25 | Graph BFS tested, watch debounce functional |
...
jarvis's vendored `scip_pb2.py` was generated from scip.proto v0.7.0 with no `typed_range` field
...
- ✓ `jarvis index` end-to-end: language detection → indexer → scip expt-convert → zoekt-index → atomic publish → registry update
...
- `--scheme` CLI flag on both `jarvis index` and `jarvis watch` — specify Xcode scheme for repos with multiple schemes
...
Fixed upstream in `scip-swift` v0.1.2, not in jarvis: `XcodebuildBuildRunner`'s `xcodebuild`
...
signing is dead weight on that path. jarvis's only change is the `SCIP_SWIFT_VERSION` pin in
...
checkout) — `jarvis status` correctly reports `failed`, no index published, atomic publish
...
- `semantic.py` — `SemanticStore` (one LanceDB table per repo under `~/.jarvis/lancedb/`,
...
New features:
- `--semantic-include` CLI flag (repeatable) on `jarvis index` — force-include a path prefix the generated-file filter would otherwise skip
...
- `--language <name>` CLI flag on both `jarvis index` and `jarvis watch` — bypasses
...
- `--search-only` CLI flag on `jarvis index` and `jarvis watch`: explicit upfront choice to skip SCIP indexing
...
Published jarvis as a Claude Code plugin (marketplace discovery) and to the official MCP Registry, enabling installation via three channels: PyPI, plugin marketplace, and MCP Registry direct listing.
...
**Why not:** jarvis is a single-user, personal tool. The MCP stdio interface is inherently local-first. Hosting would require:
...
**Why not:** jarvis is a **query engine**, not an analytics backend. A dashboard would require:
...
**Why not:** jarvis detects language by file-extension plurality and indexes one language per repo. Multi-language monorepos would require:
...
**Why not:** jarvis is designed for integration with Claude Code and Cursor, where the IDE handles the UI. A standalone GUI would require:
...
**Verdict:** Full rebuild on `jarvis reindex` is simple, reliable, and fast enough (5-30s for most repos). Not worth the complexity.

---

### Custom Symbol Filtering / Search Ranking

**Why not:** jarvis delegates to SCIP (for navigation) and Zoekt (for search) as-is. Custom filtering/ranking would require:
- Configuration format (YAML, .jarvis file, env vars)
...
**Future candidates:**
- `JARVIS_ZOEKT_BIN` — override zoekt-webserver path
- `JARVIS_SCIP_BIN` — override scip binary path
- `JARVIS_INDEXER_TIMEOUT` — timeout for language indexers (default: no timeout)
...
**Status:** Not planned, and no longer needed for disk reasons. A `jarvis gc` command would
...
- **AI code generation** — out of scope (jarvis is navigation, not generation)
...
5. Consider renaming to avoid confusion with personal jarvis

---

## Long-Term Vision (Hypothetical)

If jarvis were to grow significantly (large number of users, cross-organization use), a natural evolution might be:
