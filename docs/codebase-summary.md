...
| `.github/workflows/publish-mcp-registry.yml` | `workflow_run` on publish-pypi completion | Publishes `server.json` to official MCP Registry (`io.github.jarvis-intelligence/jarvis`); runs after PyPI publish succeeds; retries publish up to 6x for eventual consistency |
