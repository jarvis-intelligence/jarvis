...
**Files:**
- Modify: `src/jarvis/registry.py:40-117` (replace five `_ensure_*_column` functions), `:131-162` (`RegisteredRepo`, `_row_to_repo`), `:165-180` (`__init__`), `:233-248` (`get`, `list`)
...
**Files:**
- Modify: `src/jarvis/index_cli.py` (add after `_git_tracked_files`, currently ending at `:257`)
...
**Files:**
- Modify: `src/jarvis/index_cli.py` (add near `_remove_zoekt_shards`, currently `:787-804`; wire into `_cmd_forget` at `:824`)
- Test: `tests/test_index_cli.py`
...
**Files:**
- Modify: `src/jarvis/index_cli.py:575-586` (`index_repo`, right after slug resolution)
- Test: `tests/test_index_cli.py`
...
**Files:**
- Modify: `src/jarvis/index_cli.py:292-299` (`_run` returns the process), `:371-381` (delete `_write_zoekt_meta`), `:533-541` (`_publish_search_only`), `:678-685` (`index_repo`)
- Test: `tests/test_index_cli.py`
...
**Files:**
- Modify: `src/jarvis/index_cli.py` (add helpers near `_zoekt_index_cmd`; call from both publish paths)
- Test: `tests/test_index_cli.py`
...
**Files:**
- Modify: `src/jarvis/search.py` (add a method to `ZoektLifecycle` after `base_url` at `:121-122`; add a module function after `search_zoekt` at `:51`)
...
def test_zoekt_repo_documents_reads_the_list_api():
    ...
    from jarvis.search import zoekt_repo_documents
...
def test_zoekt_repo_documents_raises_on_transport_failure():
    ...
    from jarvis.search import ZoektUnavailableError, zoekt_repo_documents
...
**Files:**
- Modify: `src/jarvis/server.py:227-238` (`get_index_status`), plus a helper near `_registry_status` at `:66-80`
...
Expected: FAIL — `AttributeError: module 'jarvis.server' has no attribute '_search_coverage_fields'`.
...
```bash
git add src/jarvis/server.py tests/test_server_tools.py tests/test_index_status.py
...
Run: `grep -n "zoekt-index" .github/workflows/build-zoekt.yml setup.sh src/jarvis/*.py`
...
```bash
uv run jarvis list | awk -F'\t' '$2 == "indexed" || $2 == "partial" || $2 == "search-only" {print $1}' \
  | while read -r slug; do uv run jarvis reindex "$slug"; done
```

- [ ] **Step 5: Verify**

```bash
du -sh ~/.jarvis/.zoekt          # expect far below 2.2 GB
ls ~/.jarvis/.zoekt | wc -l      # expect roughly one shard per repo
find ~/.jarvis/.zoekt -name '*.tmp' | wc -l   # expect 0
```
...
| Coverage math is exact | jarvis: 133 tracked blobs → "attempting to index 133" → 133 processed |
| Size win | 241 MB / 7353 docs → 6.4 MB / 133 docs, 1 shard |
| Tiny `-shard_limit` forces multi-shard | `-shard_limit 120`, 6 files → 2 shards |
| `/api/list` returns per-repo `Documents` | jarvis: `Documents = 7353, Shards = 1` |
