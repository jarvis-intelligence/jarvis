...
**Goal:** Make `scip-swift`'s `xcodebuild` backend disable code signing so `codeintel index` stops failing on Swift repos with signed app-extension targets.

**Architecture:** Two repos. In `~/Projects/scip-swift` (a fork under the same ownership, pinned by `codeintel`'s `setup.sh`), extract `XcodebuildBuildRunner`'s argument list into a pure computed property so it becomes testable, then append four code-signing-disable build settings. Release that as `v0.1.2`. In `~/Projects/codeintel`, bump the version pin and one test string. No `codeintel` Python source changes, no new CLI flag, no registry migration.
...
- **`codeintel` commands:** `uv run pytest`, `uv run pytest -m "not integration"`.
...
| 4 | codeintel | Version pin bumped, asset-name test fixed, roadmap updated |
...
### Task 4: Pin `codeintel` to `v0.1.2`

**Files:**
- Modify: `~/Projects/codeintel/setup.sh:25-28`
- Modify: `~/Projects/codeintel/tests/test_setup_sh.py:624`
- Modify: `~/Projects/codeintel/docs/project-roadmap.md`
...
```bash
cd ~/Projects/codeintel && uv run pytest tests/test_setup_sh.py::test_scip_swift_asset_name_uses_macos_not_darwin -v
```
...
```bash
cd ~/Projects/codeintel && uv run pytest tests/test_setup_sh.py -v
```
...
```bash
cd ~/Projects/codeintel && uv run pytest -m "not integration"
```
...
codeintel's only change is the `SCIP_SWIFT_VERSION` pin in `setup.sh`. No new CLI flag, no
...
- Produces: a `luz-epost-ios` entry in the codeintel registry, and the verified/unverified verdict recorded in the roadmap.
...
```bash
cd ~/Projects/codeintel && ./setup.sh --only scip-swift --force
scip-swift --version
```

Expected: `0.1.2`. If the version is still `0.1.1`, the `PATH` may resolve an older copy — check `which -a scip-swift` against `~/.codeintel/bin`.

- [ ] **Step 3: Index the repo that reproduced the failure**

```bash
cd ~/Projects/codeintel
uv run codeintel index /Users/ddphuong/Projects/epost-workspace/epost-app/luz_epost_ios \
...
```bash
cd ~/Projects/codeintel && uv run codeintel status luz-epost-ios
```
...
Publishing and being queryable are different claims. Use the `codeintel` MCP tool `documentSymbols`
...
requires an `IndexConnectionCache`); use `uv run codeintel status luz-epost-ios` as a weaker
...
```markdown
Verified end-to-end: `codeintel index` completes on `luz_epost_ios` (5 signed targets),
`codeintel status` reports `language: swift` / `indexed`, and `documentSymbols` returns results.
```
...
- `uv run pytest -m "not integration"` passes in `codeintel`.
- `scip-swift --version` reports `0.1.2` and both release assets are downloadable.
- No `codeintel` Python source file, CLI parser, or registry schema was modified — `git diff main --stat` on `codeintel` touches only `setup.sh`, `tests/test_setup_sh.py`, and `docs/`.
