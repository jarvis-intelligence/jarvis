...
**Goal:** Make `jarvis index` succeed on Swift repos that ship a checked-in `.xcodeproj`/`.xcworkspace` alongside `Package.swift` (e.g. UIKit-only iOS packages that can't build with plain `swift build`), by forcing `scip-swift`'s `--build-tool xcodebuild` in that case, with an explicit, persisted `--scheme` override for repos with more than one scheme.

**Architecture:** Two small, pure, unit-testable helpers in `index_cli.py` — `_prefers_xcodebuild()` (filesystem check) and `_swift_indexer_cmd()` (command-list builder) — wired into the existing `index_repo()` pipeline. A new nullable `scheme_override` column on `registry.py`'s `repos` table persists the `--scheme` value so `jarvis reindex`/`jarvis watch` don't need it repeated. No changes to `scip-swift` itself.

**Tech Stack:** Python 3.14, stdlib `sqlite3`, `argparse`, `pytest` (existing jarvis stack — no new dependencies).
...
In `src/jarvis/index_cli.py`, add directly below `detect_language()` (after its closing `return _LANGUAGE_INDEXERS[best_ext]` at line 87):
...
In `src/jarvis/index_cli.py`, add directly below `_prefers_xcodebuild` (from Task 2):
...
- Produces: `jarvis index <path> --scheme <name>`; `jarvis watch <path> --scheme <name>`; `jarvis reindex <slug>` now passes the stored `scheme_override` through automatically.
...
In `src/jarvis/index_cli.py`, update `_cmd_index` (currently):
...
```
uv run jarvis index /path/to/repo [--slug name]
```

to:

```
uv run jarvis index /path/to/repo [--slug name] [--scheme name]
```

and change:

```
uv run jarvis watch /path/to/repo [--debounce 5]   # foreground, not a daemon
```

to:

```
uv run jarvis watch /path/to/repo [--debounce 5] [--scheme name]   # foreground, not a daemon
```

In the `## Architecture` section, right after the existing **Known gap** paragraph (`... not a bug in jarvis's query logic.`), add a new paragraph:
...
- [ ] **Step 2: Verify via `jarvis status`**

```bash
uv run jarvis status epost-ios-theme-ui
```
...
Call `mcp__jarvis__getIndexStatus` with `repo="epost-ios-theme-ui"` and
...
Call `mcp__jarvis__searchCode` with `query="ThemeColorPicker"` and `repo="epost-ios-theme-ui"`.
