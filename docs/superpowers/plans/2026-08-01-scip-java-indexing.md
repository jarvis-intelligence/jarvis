...
**Architecture:** `setup.sh` installs upstream's single-file JVM launcher into `~/.codeintel/bin`. `index_cli.py` serializes Gradle for Java (an upstream concurrency bug) and, when the indexer fails with a *recognized* signature, degrades to a search-only publish (Zoekt + semantic, no SCIP) and persists that decision so later runs skip the doomed build. The chunker's existing fixed-window fallback is opened to more languages so search-only repos get semantic coverage.
...
- **`setup.sh` is STRICTLY POSIX sh.** No arrays, no `[[ ]]`, no bashisms — `curl | sh` runs under dash. Tests source it with `CODEINTEL_SETUP_SOURCED=1` and run under `dash`.
...
| `src/codeintel/index_cli.py` | `_run` env param, Java `GRADLE_OPTS`, `--search-only`, signature fallback | 2, 4, 5 |
| `src/codeintel/registry.py` | `search_only` column + migration | 3 |
| `src/codeintel/chunker.py` | Extend `LANGUAGES` allowlist | 6 |
| `src/codeintel/server.py` | Search-only error text, `status` in `getIndexStatus` | 7 |
| `tests/fixtures/mini_java_repo/` | Gradle fixture for the integration test | 8 |
| `CLAUDE.md`, `CHANGELOG.md`, `.claude/skills/codeintel-setup/SKILL.md` | Docs | 9 |
...
**Files:**
- Modify: `src/codeintel/index_cli.py` (`_run` ~line 201; indexer invocation ~line 433)
...
Replace `_run` in `src/codeintel/index_cli.py`:
...
**Files:**
- Modify: `src/codeintel/registry.py` (`_SCHEMA` ~line 19; migrations ~line 95; `RegisteredRepo` ~line 112; `_row_to_repo` ~line 125; `upsert` ~line 158; both `SELECT` statements at lines 207 and 216)
...
At the top of `src/codeintel/registry.py`, below the imports:
...
In `src/codeintel/registry.py`, add a final column to the `_SCHEMA` `CREATE TABLE`:
...
**Files:**
- Modify: `src/codeintel/index_cli.py` (constants ~line 71; resolvers ~line 300; `index_repo` ~line 402; `_cmd_index` ~line 493; parser ~line 680)
- Test: `tests/test_index_cli.py`
...
In `src/codeintel/index_cli.py`, below `PARTIAL_STATUS`:
...
**Files:**
- Modify: `src/codeintel/index_cli.py` (signature table near `SEARCH_ONLY_STATUS`; indexer call in `index_repo`)
- Test: `tests/test_index_cli.py`
...
Below `UNKNOWN_LANGUAGE` in `src/codeintel/index_cli.py`:
...
**Files:**
- Modify: `src/codeintel/chunker.py` (`LANGUAGES` ~line 62)
...
Replace the `LANGUAGES` dict in `src/codeintel/chunker.py`:
...
**Files:**
- Modify: `src/codeintel/server.py` (error returns at lines 70, 81, 92, 103, 123; `get_index_status` ~line 144)
...
Expected: FAIL with `AttributeError: module 'codeintel.server' has no attribute '_error_payload'`.

- [ ] **Step 3: Implement the helper**

Add to `src/codeintel/server.py`, above the first `@mcp.tool`:
...
**Files:**
- Modify: `CLAUDE.md`, `CHANGELOG.md`, `.claude/skills/codeintel-setup/SKILL.md`
...
In `.claude/skills/codeintel-setup/SKILL.md`, replace `scip-java` (detect-only)` in the installed-binaries sentence with:
...
```bash
git add CLAUDE.md CHANGELOG.md .claude/skills/codeintel-setup/SKILL.md
...
- **Sentinel language:** `UNKNOWN_LANGUAGE = "unknown"`, shown verbatim by `codeintel list` and
  `status`.
