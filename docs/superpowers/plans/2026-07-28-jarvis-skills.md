# jarvis Skill Toolkit Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Ship three `jarvis-*` agent skills (setup / use / issues) plus a Python helper that symlinks them into `~/.zcode/skills/`, compliant with `ck:skill-creator` v4.0.0 (eval harness deferred).

**Architecture:** Three self-contained skills under `.claude/skills/` (the standard's required location). Each is a single `SKILL.md` (<300 lines) with ≤200-char pushy descriptions. `jarvis-use` spills 8-tool detail to `references/tool-roster.md` (progressive disclosure). A Python helper `scripts/link_skills.py` symlinks each skill dir into `~/.zcode/skills/` so a ZCode agent loads them; repo files remain the single source of truth.

**Tech Stack:** Markdown + YAML frontmatter (skills); Python 3 stdlib only (`link_skills.py`); pytest (test).

## Global Constraints

- Skills live in `.claude/skills/jarvis-<name>/SKILL.md` (NOT `~/.claude/skills/`, NOT repo-root `skills/`).
...
├── jarvis-use/
│   ├── SKILL.md                 # everyday decision matrix + prefer-jarvis rule (Task 3)
│   └── references/
│       └── tool-roster.md       # 8-tool detail, progressive disclosure (Task 3)
└── jarvis-issues/
    └── SKILL.md                 # file jarvis bugs via gh (Task 4)
scripts/
...
**Responsibilities:**
- `link_skills.py` — sole executable; idempotently links `.claude/skills/jarvis-*` → `~/.zcode/skills/`.
- `jarvis-setup/SKILL.md` — linear install → register → index → verify runbook. Owns onboarding.
- `jarvis-use/SKILL.md` — decision matrix + freshness flow; owns everyday structural queries.
- `jarvis-use/references/tool-roster.md` — per-tool signatures/returns; loaded on demand.
- `jarvis-issues/SKILL.md` — gather/classify/draft/file flow; owns the feedback loop.
...
- Produces: `link_skills(repo_root: Path, target_dir: Path) -> list[str]` — creates one symlink per `.claude/skills/jarvis-*` dir into `target_dir`, returns the list of created symlink paths. Idempotent: skips names already linked. CLI entry `python -m`-style `main()` prints a summary and exits 0.
...
```python
"""Tests for scripts/link_skills.py — symlink helper for jarvis skills."""
...
def test_creates_symlink_for_each_jarvis_skill(tmp_path):
    link_skills = _load_link_skills()
    repo_root = tmp_path / "repo"
    target = tmp_path / "target"
    for name in ("jarvis-setup", "jarvis-use", "jarvis-issues"):
        _make_skill(repo_root, name)
    # a non-jarvis skill must be ignored
...
def test_idempotent_skips_existing_links(tmp_path):
    ...
    repo_root = tmp_path / "repo"
    target = tmp_path / "target"
    _make_skill(repo_root, "jarvis-setup")
    ...
    assert (target / "jarvis-setup" / "SKILL.md").exists()  # still linked
...
def test_replaces_broken_symlink(tmp_path):
    ...
    target.mkdir()
    (target / "jarvis-setup").symlink_to(tmp_path / "does-not-exist")

    created = link_skills.link_skills(repo_root, target)

    assert len(created) == 1
    link = target / "jarvis-setup"
    assert link.is_symlink()
...
```python
"""Symlink jarvis skills (.claude/skills/jarvis-*) into a target dir.
...
so this creates one symlink per jarvis skill there. The repo file stays the
...
def link_skills(repo_root: Path, target_dir: Path) -> list[str]:
    """Create one symlink per `.claude/skills/jarvis-*` dir in target_dir.
...
def main(argv: list[str] | None = None) -> int:
    ...
    if created:
        ...
        print(f"All jarvis skills already linked in {args.target}.")
    return 0
...
Run: `uv run pytest tests/test_link_skills.py -v`
Expected: PASS — 3 tests (`test_creates_symlink_for_each_jarvis_skill`, `test_idempotent_skips_existing_links`, `test_replaces_broken_symlink`).
...
Idempotently links .claude/skills/jarvis-* into ~/.zcode/skills/ so a
ZCode agent loads them; repo files stay the single source of truth."
```

---

### Task 2: `jarvis-setup` skill

**Files:**
- Create: `.claude/skills/jarvis-setup/SKILL.md`

**Interfaces:**
- Consumes: jarvis's documented install/CLI surface (`setup.sh`, `uv sync`, `jarvis index`, `jarvis status`; CLI flags `--slug`, `--scheme`).
- Produces: a skill that triggers on onboarding intents. Points users to `jarvis-use` (Task 3) and `jarvis-issues` (Task 4) by name.
...
Cursor / other MCP clients: point them at the stdio command `uv --directory /path/to/jarvis run jarvis-server`. No HTTP server, no auth, no network.

## 4. Index a repo

```bash
uv run jarvis index /path/to/your/repo            # slug = directory name
uv run jarvis index /path/to/your/repo --slug foo # explicit slug
uv run jarvis index /path/to/your/repo --scheme MyScheme  # Swift, ambiguous Xcode scheme
```
...
```bash
uv run jarvis status <slug>     # expect status: indexed
```
...
| Symptom | Fix |
|---|---|
| `command not found: scip` / `zoekt-index` | `~/.jarvis/bin` not on `PATH`. Open a new shell, or `source ~/.zshrc` (or `~/.bashrc`). |
...
```bash
f=.claude/skills/jarvis-setup/SKILL.md
...
```bash
git add .claude/skills/jarvis-setup/SKILL.md
git commit -m "feat(skills): add jarvis-setup onboarding skill"
```

---

### Task 3: `jarvis-use` skill + tool roster

**Files:**
- Create: `.claude/skills/jarvis-use/SKILL.md`
- Create: `.claude/skills/jarvis-use/references/tool-roster.md`

**Interfaces:**
- Consumes: the 8 MCP tools registered in `src/jarvis/server.py` (`documentSymbols`, `goToDefinition`, `findReferences`, `callHierarchy`, `typeHierarchy`, `getIndexStatus`, `searchCode`, `blastRadius`) and the CLI (`jarvis reindex <slug>`).
...
---
name: jarvis-use
description: Use jarvis MCP tools for code structure queries: finding references, go-to-definition, call/type hierarchy, who calls a function, where a symbol is defined, document symbols. Prefer over grep.
version: "0.1.0"
---
...
2. Branch on the result:
   ...
   - **indexed + stale** → run `uv run jarvis reindex <slug>`, then call the tool.
...
The 8 MCP tools registered by `jarvis-server`. All take `repo` (the slug from `jarvis index`). On failure every tool returns `{"error": "..."}` rather than raising.
...
```bash
main=.claude/skills/jarvis-use/SKILL.md
ref=.claude/skills/jarvis-use/references/tool-roster.md
...
```bash
git add .claude/skills/jarvis-use/SKILL.md .claude/skills/jarvis-use/references/tool-roster.md
git commit -m "feat(skills): add jarvis-use skill + tool roster

Decision matrix prefers jarvis over grep for structural queries;
getIndexStatus-first freshness flow; 8-tool detail in references/."
```

---

### Task 4: `jarvis-issues` skill

**Files:**
- Create: `.claude/skills/jarvis-issues/SKILL.md`

**Interfaces:**
- Consumes: `gh` CLI (assumes installed + authed), jarvis's CLI (`jarvis status`), the package version via `importlib.metadata`, the known-limitations list (embedded).
- Produces: a skill that triggers on jarvis-bug/feature intents. Always confirms with the user before `gh issue create`.
...
- Repo + slug, and `uv run jarvis status <slug>` output.
...
- jarvis version: `uv run python -c "import importlib.metadata; print(importlib.metadata.version('jarvis'))"` (jarvis has no `--version` flag; this reads it from package metadata).
...
```bash
f=.claude/skills/jarvis-issues/SKILL.md
desc=$(awk '/^description:/{sub(/^description: /,""); print; exit}' "$f")
...
```bash
git add .claude/skills/jarvis-issues/SKILL.md
git commit -m "feat(skills): add jarvis-issues skill
...
Expected output: `Linked 3 skill(s) into /Users/<you>/.zcode/skills/:` followed by the three `jarvis-*` paths. (If run before, `All jarvis skills already linked in ...`.)

- [ ] **Step 2: Verify all three resolve through the symlinks**

```bash
for s in jarvis-setup jarvis-use jarvis-issues; do
...
Three agent skills in `.claude/skills/` help onboard and use jarvis:
...
```bash
git add README.md
git commit -m "docs(skills): document the jarvis skill toolkit in README"
```
...
| `jarvis-setup` (7 sections) | Task 2 |
| `jarvis-use` decision matrix + freshness flow + gotchas | Task 3 |
| `jarvis-use/references/tool-roster.md` (8 tools) | Task 3 |
| `jarvis-issues` (gather/classify/draft/file, confirm-before-submit, known-limitations) | Task 4 |
...
| Eval harness | **Explicitly deferred** (Global Constraints + lightweight trigger-examples in `jarvis-use`) |
...
**3. Type consistency:** `link_skills(repo_root: Path, target_dir: Path) -> list[str]` is defined in Task 1 Step 3 and called consistently. Skill names `jarvis-setup` / `jarvis-use` / `jarvis-issues` are identical across all tasks and match the `SKILL_PREFIX = "jarvis-"` filter in Task 1. CLI flags `--slug` / `--scheme` match `src/jarvis/index_cli.py:486-488`. Tool names match `src/jarvis/server.py` decorators.
