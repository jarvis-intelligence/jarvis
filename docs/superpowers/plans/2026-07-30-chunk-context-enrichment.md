...
**Files:**
- Modify: `src/jarvis/chunker.py` (constants at lines 25-28; `Chunk` at 71-80; `_make_chunk` at 132; `_split_class` at 229-255; `_merge_small` at 258-274; delete `_collect_imports` at 277-293; `chunk_file` at 296-321; `_window_lines` line 132 sizing)
...
Add `MAX_TOKENS` to the existing import from `jarvis.chunker` at line 7 if not already present (it is).
...
In `src/jarvis/chunker.py`, replace lines 25-28:
...
**Files:**
- Modify: `src/jarvis/chunker.py` (add predicates after `skip_reason`, ends line 119; `iter_source_files` at 122-129)
- Modify: `src/jarvis/semantic.py` (loop at lines 184-202)
...
In `src/jarvis/chunker.py`, add `import subprocess` to the imports at the top, add the constant beside the other admission constants (after `MAX_LINE_CHARS = 5000`, line 43):
...
In `src/jarvis/semantic.py`, extend the chunker import (line 15) to include `oversized_file_reason`, then replace the first three lines of the loop body (lines 185-186):
...
```bash
git add src/jarvis/chunker.py src/jarvis/semantic.py tests/test_chunker.py tests/test_semantic.py
...
**Files:**
- Modify: `src/jarvis/embeddings.py` (constants near line 21; `__init__` at 30-39; `embed_texts` at 56-64; `count_oversized` at 66-83; `embed_query` at 85-88)
...
- Produces:
  ...
  - Env vars `JARVIS_EMBEDDING_QUERY_PREFIX`, `JARVIS_EMBEDDING_DOC_PREFIX`.
...
In `src/jarvis/embeddings.py`, after `MAX_SEQ_LENGTH = 1024` (line 21):
...
**Files:**
- Modify: `src/jarvis/semantic.py` (imports line 15-16; dataclasses near line 83; `table_identity` at 126-133; `index_semantic` at 166-228; `semantic_search` at 231-247)
...
In `src/jarvis/semantic.py`, extend the chunker import (line 15) to include `CONTENT_FORMAT`, and add after the `SkippedFile` dataclass:
...
```python
    ...
    if configured != identity:
        ...
        warning = (f"configured embedding model {configured.model_name}@"
                   ...
                   f"queried with the index's — run jarvis reindex {slug} to migrate")
...
**Files:**
- Modify: `src/jarvis/semantic.py` (`SemanticIndexReport` near line 89; `index_semantic` return near line 227)
- Modify: `src/jarvis/index_cli.py` (`_print_semantic_report`)
...
In `src/jarvis/semantic.py`, add before `SemanticIndexReport`:
...
In `src/jarvis/index_cli.py`, add to `_print_semantic_report`, after the existing skipped-file loop and before the truncation branch:
...
```bash
git add src/jarvis/semantic.py src/jarvis/index_cli.py tests/test_semantic.py tests/test_index_cli.py
...
```bash
uv run jarvis index . --slug selfcheck 2>&1 | grep "chunk tokens"
```

Expected: a line like `semantic: chunk tokens p50=… p90=… max=…`. Then clean up: `uv run jarvis forget selfcheck`.
