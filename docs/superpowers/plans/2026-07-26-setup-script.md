...
**Goal:** Ship a single curl-installable `setup.sh` that takes a bare macOS or Linux machine to a fully-working `jarvis` install by detecting and installing all 7 external binary dependencies.

**Architecture:** One POSIX-`sh` script at repo root with one isolated installer function per dependency, each independently failing-soft. Binaries land in `~/.jarvis/bin`, which the script idempotently adds to the user's shell rc. Because upstream `sourcegraph/zoekt` publishes no binaries at all, a new GitHub Actions workflow cross-compiles zoekt from a pinned commit and publishes it to jarvis's own releases, which `setup.sh` then downloads from.
...
- **Install dir:** `~/.jarvis/bin` (override via `JARVIS_BIN_DIR` for tests).
...
| `.github/workflows/build-zoekt.yml` (create) | Cross-compiles `zoekt-index` + `zoekt-webserver` for 4 os/arch pairs, smoke-tests, publishes to jarvis releases |
| `tests/test_setup_sh.py` (create) | pytest tests that source `setup.sh` with `JARVIS_SETUP_SOURCED=1` and assert individual function behavior |
...
- Produces:
  ...
  - Sourced-guard honoring `JARVIS_SETUP_SOURCED=1`
...
Run: `sh setup.sh`
Expected: prints `jarvis setup` then `platform: darwin/arm64`, exit 0.
...
- Produces:
  - `bin_dir()` → echoes `$JARVIS_BIN_DIR` if set, else `$HOME/.jarvis/bin`
...
JARVIS_SETUP_SOURCED=1 JARVIS_BIN_DIR=/tmp/ci-scip-test FORCE=1 \
  sh -c '. ./setup.sh; install_scip darwin arm64'
/tmp/ci-scip-test/scip --version
```
(`JARVIS_SETUP_SOURCED=1` is required so sourcing doesn't trigger `main`.)
...
go mod init zoekt-build
GOFLAGS=-mod=mod go get "github.com/sourcegraph/zoekt@$(tr -d '[:space:]' < ~/Projects/jarvis/ZOEKT_COMMIT)"
...
JARVIS_SETUP_SOURCED=1 JARVIS_BIN_DIR=/tmp/ci-zoekt-test FORCE=1 \
...
```bash
git add setup.sh tests/test_setup_sh.py
git commit -m "feat(setup): add zoekt installer sourced from jarvis releases"
```
...
def test_install_scip_java_reports_and_returns_zero_without_docker(tmp_path):
    ...
    result = subprocess.run(
        ...
        capture_output=True,
        text=True,
        env={"JARVIS_SETUP_SOURCED": "1", "PATH": f"{empty_bin}:/usr/bin:/bin"},
        stdin=subprocess.DEVNULL,
    )
    assert result.returncode == 0
    combined = (result.stdout + result.stderr).lower()
...
def test_install_scip_java_never_pulls_without_confirmation(tmp_path):
    ...
    result = subprocess.run(
        ...
        capture_output=True,
        text=True,
        env={"JARVIS_SETUP_SOURCED": "1", "PATH": f"{fake_bin}:/usr/bin:/bin"},
        stdin=subprocess.DEVNULL,
    )
    assert result.returncode == 0
...
`setup.sh` installs every external binary jarvis needs into `~/.jarvis/bin`
...
Run:
```bash
rm -rf /tmp/jarvis-e2e
JARVIS_BIN_DIR=/tmp/jarvis-e2e sh setup.sh
ls -la /tmp/jarvis-e2e
/tmp/jarvis-e2e/scip --version
/tmp/jarvis-e2e/zoekt-webserver -version
/tmp/jarvis-e2e/scip-swift --version
```
Expected: summary shows `ok` for scip, zoekt, scip-swift; each binary runs. `scip-java` reports detect-only.

- [ ] **Step 8: Verify the pinned `scip v0.9.0` actually works with jarvis**

This is the empirical check the spec requires — the schema was proven identical to v0.7.0, but v0.9.0 had not been executed against jarvis.

Run:
```bash
PATH="/tmp/jarvis-e2e:$PATH" uv run pytest -m integration -v
```
Expected: PASS.
...
docker run --rm -v "$PWD:/w" -w /w debian:stable-slim sh -c '
  ...
  JARVIS_BIN_DIR=/tmp/ci-bin sh setup.sh --only scip
...
docker run --rm -v "$PWD:/w" -w /w debian:stable-slim sh -c '
  apt-get update -qq && apt-get install -y -qq curl ca-certificates tar >/dev/null
  JARVIS_BIN_DIR=/tmp/ci-bin sh setup.sh --only zoekt
...
**Known limitation carried forward, not introduced here:** `index_cli.py` invokes `zoekt-index` without passing a repo name, so Zoekt names shards by directory basename rather than by jarvis's `--slug`. When a slug differs from the directory name, `searchCode(repo=<slug>)`'s `r:` filter silently matches nothing. This is a pre-existing jarvis bug, out of scope for this plan, and worth a separate fix.
