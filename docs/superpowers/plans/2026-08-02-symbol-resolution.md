...
**Files:**
- Create: `src/jarvis/symbols.py`
- Create: `tests/test_symbols.py`

**Interfaces:**
- Consumes: nothing (pure text, no DB, no other jarvis module).
- Produces:
...
from jarvis.symbols import DescriptorKind, ParsedSymbol, parse_symbol

# Real symbols captured from live indexes: Python (jarvis itself) and
# TypeScript (tests/fixtures/synthetic_index.py).
PY_METHOD = (
    "scip-python python jarvis-mcp 0.3.1 "
    "`jarvis.search`/search_zoekt()."
)
PY_PARAM = (
    "scip-python python jarvis-mcp 0.3.1 "
    "`jarvis.search`/search_zoekt().(base_url)"
)
PY_FIELD = (
    "scip-python python jarvis-mcp 0.3.1 "
    "`jarvis.search`/ZoektHit#repo."
)
...
def test_dotted_path_leads_with_the_package():
    ...
    assert parsed.dotted_path == (
        "jarvis-mcp.jarvis.search.search_zoekt"
    )
...
def test_parses_parameter_descriptor():
    ...
    assert parsed.parents == ("jarvis.search", "search_zoekt")
...
def test_parses_term_descriptor():
    ...
    assert parsed.parents == ("jarvis.search", "ZoektHit")
...
Expected: FAIL — `ModuleNotFoundError: No module named 'jarvis.symbols'`
...
from jarvis.symbols import parse_symbol, DescriptorKind
skip = {DescriptorKind.PARAMETER, DescriptorKind.TYPE_PARAMETER}
base = os.path.expanduser('~/.jarvis/scip/_')
...
```bash
git add src/jarvis/symbols.py tests/test_symbols.py docs/superpowers/specs/2026-08-02-symbol-resolution-design.md
...
**Files:**
- Modify: `src/jarvis/symbols.py` (append; do not change Task 1's code)
...
def test_a_method_wins_over_a_same_named_parameter_elsewhere():
    ...
    py_method_base_url = (
        "scip-python python jarvis-mcp 0.3.1 "
        "`jarvis.search`/ZoektLifecycle#base_url()."
    )
...
Expected: FAIL — `ImportError: cannot import name 'resolve' from 'jarvis.symbols'`

- [ ] **Step 3: Append the resolver**

Append to `src/jarvis/symbols.py`:
...
**Files:**
- Modify: `src/jarvis/query.py` (imports; `QueryService` methods at lines 337, 346, 405, 413)
...
In `src/jarvis/query.py`, add to the import block:
...
```bash
git add src/jarvis/query.py tests/test_query.py
...
**Files:**
- Modify: `src/jarvis/server.py` (`_error_payload` at line 82; tools at lines 109, 120, 131, 147)
...
In `src/jarvis/server.py`, add to the import block:
...
**Files:**
- Modify: `src/jarvis/query.py` (`_symbol_display_and_kind` at line 172, `_symbol_info` at line 179, `get_document_symbols` at lines 370-379 and 386-400)
- Modify: `tests/test_query.py` (append)
...
In `src/jarvis/query.py`, add after `_symbol_display_and_kind` (line 176):
...
```bash
uv run jarvis reindex jarvis
```

Then via the MCP client (or `QueryService` directly):

1. `findReferences(repo="jarvis", symbol="search_zoekt")` → references, not `[]`
2. same call with the full SCIP string → identical result
3. `findReferences(repo="jarvis", symbol="__init__")` → `candidates` + `candidateTotal`
4. `findReferences(repo="jarvis", symbol="tmp_path")` → error naming the parameter rule.
...
- **Acceptance criteria 1, 2, 3, 6 verified against the live `jarvis` index.**
