...
- Heavy imports (`tree_sitter_language_pack`, `sentence_transformers`, `lancedb`) are deferred inside functions — importing `jarvis.semantic`/`chunker`/`embeddings` must succeed without the extra installed.
- Env vars prefixed `JARVIS_`: `JARVIS_EMBEDDING_MODEL` (default `BAAI/bge-m3`), `JARVIS_EMBEDDING_BATCH_SIZE` (default 32).
...
- Modify: `src/jarvis/config.py` (add `IGNORED_DIRS`, `lancedb_dir()`)
- Modify: `src/jarvis/index_cli.py:46` (alias `_IGNORED_DIRS` to config)
...
- [ ] **Step 3: Implement `src/jarvis/chunker.py`**
...
- [ ] **Step 5: Commit** — `git add src/jarvis/chunker.py tests/test_chunker.py && git commit -m "feat(semantic): tree-sitter AST chunker with fixed-window fallback"`
...
**Files:**
- Create: `src/jarvis/embeddings.py`
...
- [ ] **Step 4: Implement `src/jarvis/embeddings.py`**
...
- [ ] **Step 6: Commit** — `git add src/jarvis/embeddings.py tests/test_embeddings.py && git commit -m "feat(semantic): lazy sentence-transformers embedding wrapper with pinned revision"`
...
**Files:**
- Create: `src/jarvis/semantic.py`
- Test: `tests/test_semantic.py`

**Interfaces:**
- Consumes: `ZoektHit` from `jarvis.search` (fields `repo, path, line_number, line_text`).
- Produces:
...
from jarvis.search import ZoektHit
...
- [ ] **Step 3: Implement** the top of `src/jarvis/semantic.py`:
...
from jarvis.chunker import Chunk, chunk_file, hash_file, iter_source_files, language_for
...
- [ ] **Step 5: Commit** — `git add src/jarvis/semantic.py tests/test_semantic.py && git commit -m "feat(semantic): reciprocal rank fusion of vector and zoekt hits"`
...
- [ ] **Step 5: Commit** — `git add src/jarvis/semantic.py tests/test_semantic.py && git commit -m "feat(semantic): lancedb store with file carry-over and model-identity rule"`
...
**Interfaces:**
- Consumes: `SemanticStore`, `reciprocal_rank_fusion` (Tasks 4-5); `search_zoekt(base_url, query)` and `ZoektUnavailableError` from `jarvis.search`.
...
Also add the imports the new tests need at the top of the test file: `from jarvis.search import ZoektHit, ZoektUnavailableError`.
...
def semantic_search(slug: str, query: str, limit: int = 10, *, root: Path | None = None,
    ...
    if model.identity() != identity:
        ...
        warning = (f"configured embedding model {model.identity()[0]}@{model.identity()[1]} "
                   ...
                   f"index's model — run jarvis reindex {slug} to migrate")
...
- [ ] **Step 5: Commit** — `git add src/jarvis/semantic.py tests/test_semantic.py && git commit -m "feat(semantic): hybrid semantic_search with table-model authority and zoekt fallback"`
...
**Files:**
- Modify: `src/jarvis/registry.py`
- Modify: `src/jarvis/index_cli.py:346-347` (`_cmd_status` output)
...
- [ ] **Step 5: Commit** — `git add src/jarvis/registry.py src/jarvis/index_cli.py tests/test_registry.py && git commit -m "feat(semantic): track semantic_indexed_at in the registry"`
...
**Files:**
- Modify: `src/jarvis/index_cli.py` (new `_run_semantic_stage()`; call inside `index_repo()` after the `zoekt-index` `_run` at line 279-283 and before the publish block at line 285; `mark_semantic_indexed` after the final `registry.upsert`; lance cleanup in `_cmd_forget`)
...
def test_semantic_stage_skips_cleanly_when_extra_missing(monkeypatch, capsys):
    ...
    monkeypatch.setitem(sys.modules, "jarvis.semantic", None)  # forces ImportError
...
- [ ] **Step 5: Commit** — `git add src/jarvis/index_cli.py tests/test_index_cli.py && git commit -m "feat(semantic): non-fatal semantic stage in index pipeline and forget cleanup"`
...
**Files:**
- Modify: `src/jarvis/server.py` (new tool after `search_code`, line 177; update the module docstring's tool count/list at lines 4-5)
...
- [ ] **Step 5: Commit** — `git add src/jarvis/server.py tests/test_server_tools.py && git commit -m "feat(semantic): semanticSearch MCP tool"`
...
- [ ] **Step 3: Update docs.**
  - `CLAUDE.md`: in Architecture, add a fourth engine bullet — Semantic (`chunker.py` + `embeddings.py` + `semantic.py`): tree-sitter chunking → sentence-transformers embeddings → per-repo LanceDB table under `~/.jarvis/lancedb/`; hybrid `semanticSearch` fuses vector + Zoekt hits via RRF; optional `semantic` extra, stage is non-fatal in `jarvis index`; a table only holds vectors from one model+revision. Add `uv sync --extra semantic` to the Commands block.
...
- [ ] Manual smoke: `uv run jarvis index <some-python-repo>` prints no semantic errors; `jarvis status <slug>` shows `semantic: <timestamp>`; a `semanticSearch` call through the MCP server returns fused results.
