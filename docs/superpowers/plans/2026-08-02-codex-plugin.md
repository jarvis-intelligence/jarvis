# Codex CLI Plugin for codeintel — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Ship a Codex CLI plugin manifest + per-skill `agents/openai.yaml` files so `codeintel` installs under Codex via `codex plugin marketplace add` / `codex plugin add`, reusing the existing `plugin/` content root.
...
- Create: `plugin/skills/codeintel-setup/agents/openai.yaml` — Codex agent metadata for setup.
- Create: `plugin/skills/codeintel-use/agents/openai.yaml` — Codex agent metadata for use.
- Create: `plugin/skills/codeintel-issues/agents/openai.yaml` — Codex agent metadata for issues.
- Create: `plugin/assets/codeintel-small.svg` — placeholder `composerIcon`.
...
- Modify: `plugin/skills/codeintel-setup/SKILL.md:36-43` — §3 "Register the MCP server" becomes client-aware.
- Modify: `plugin/skills/codeintel-use/SKILL.md:63-66` — `semanticSearch` gotcha adds the Codex second-server command.
...
```json
{
  ...
  "interface": {
    ...
    "websiteURL": "https://github.com/phuongddx/codeintel",
    "privacyPolicyURL": "https://github.com/phuongddx/codeintel/blob/main/plugin/README.md#privacy",
    "termsOfServiceURL": "https://github.com/phuongddx/codeintel/blob/main/plugin/LICENSE",
    "brandColor": "#3B82F6",
    "composerIcon": "./plugin/assets/codeintel-small.svg",
...
**Files:**
- Create: `plugin/skills/codeintel-setup/agents/openai.yaml`
- Create: `plugin/skills/codeintel-use/agents/openai.yaml`
- Create: `plugin/skills/codeintel-issues/agents/openai.yaml`

- [ ] **Step 1: Create the setup agent file**

`plugin/skills/codeintel-setup/agents/openai.yaml`:
```yaml
interface:
  display_name: "codeintel Setup"
  short_description: "Install and configure codeintel for Codex"
  default_prompt: "Use $codeintel-setup to install codeintel, register the MCP server, and index a repo for the first time."
```

- [ ] **Step 2: Create the use agent file**

`plugin/skills/codeintel-use/agents/openai.yaml`:
```yaml
interface:
  display_name: "codeintel Navigation"
  short_description: "Prefer codeintel MCP tools over grep for structural queries"
  default_prompt: "Use $codeintel-use to answer structural code questions via SCIP navigation (definition, references, call hierarchy) and search."
```

- [ ] **Step 3: Create the issues agent file**

`plugin/skills/codeintel-issues/agents/openai.yaml`:
```yaml
interface:
  display_name: "codeintel Issues"
  short_description: "File bugs and feature requests for codeintel"
  default_prompt: "Use $codeintel-issues to gather context, classify against known limitations, and file a well-formed GitHub issue for codeintel."
```

- [ ] **Step 4: Verify all three parse as YAML**

Run: `uv run python -c "import yaml; [yaml.safe_load(open(f'plugin/skills/{s}/agents/openai.yaml')) for s in ('codeintel-setup','codeintel-use','codeintel-issues')]" && echo OK`
Expected: `OK` (no parse errors). If `yaml` is unavailable in the base env, run `uv run --with pyyaml python -c "..."` instead.

- [ ] **Step 5: Commit**

```bash
git add plugin/skills/codeintel-setup/agents/openai.yaml \
        plugin/skills/codeintel-use/agents/openai.yaml \
        plugin/skills/codeintel-issues/agents/openai.yaml
...
Only two skills need content edits. `codeintel-issues/SKILL.md` has no client-specific commands and is left untouched (only its new `agents/openai.yaml` from Task 3 applies).

**Files:**
- Modify: `plugin/skills/codeintel-setup/SKILL.md` (§3 "Register the MCP server", around line 36)
- Modify: `plugin/skills/codeintel-use/SKILL.md` (the `semanticSearch` gotcha, around lines 63-66)

- [ ] **Step 1: Read the exact current §3 block in the setup skill**

Run: `sed -n '36,50p' plugin/skills/codeintel-setup/SKILL.md`
...
In `plugin/skills/codeintel-setup/SKILL.md`, replace the §3 block (from the `## 3. Register the MCP server` heading through the blank line before `## 4. Index a repo`) with:
...
In `plugin/skills/codeintel-use/SKILL.md`, the gotcha block currently ends with a single Claude command:
...
Leave the surrounding explanation (why the default registration is intentionally extra-free, and that `codeintel` is already taken by the plugin's registration) unchanged.
...
```bash
git add plugin/skills/codeintel-setup/SKILL.md plugin/skills/codeintel-use/SKILL.md
...
**Files:**
- Create: `plugin/assets/codeintel-small.svg`
- Create: `plugin/assets/app-icon.png`

- [ ] **Step 1: Create the SVG**

Create `plugin/assets/codeintel-small.svg` with a minimal monochrome mark:
...
rsvg-convert -w 256 -h 256 plugin/assets/codeintel-small.svg -o plugin/assets/app-icon.png \
  || qlmanage -t -s 256 -o plugin/assets plugin/assets/codeintel-small.svg \
     && mv plugin/assets/codeintel-small.svg.png plugin/assets/app-icon.png 2>/dev/null
```
...
```bash
git add plugin/assets/codeintel-small.svg plugin/assets/app-icon.png
...
```bash
codex plugin marketplace add /Users/ddphuong/Projects/codeintel --name codeintel-local
codex plugin add codeintel@codeintel-local
codex plugin list
codex mcp list
```
Expected: `codex plugin list` shows `codeintel` installed; `codex mcp list` shows `codeintel` auto-registered (proof that Codex consumed `plugin/.mcp.json`). If `codex mcp list` is empty, the plugin install did not pick up `.mcp.json` — investigate before proceeding.
...
- **`codeintel-issues/SKILL.md` is untouched.** It has no client-specific commands (`rg "claude|codex|mcp add" plugin/skills/codeintel-issues/SKILL.md` returns nothing). Only its new `agents/openai.yaml` is added.
...
- **The release runbook is a follow-up, not a task here.** The version guard makes drift impossible; updating `.claude/skills/codeintel-release/SKILL.md` prose to say "five files" is a documentation task tracked separately.
