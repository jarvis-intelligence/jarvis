...
**Files:**
- Modify: `src/jarvis/chunker.py` (add constants after line 28 `MAX_IMPORT_LINES = 10`; add functions after `language_for()` at line 76-77)
...
Add to the end of `tests/test_chunker.py`. Note the existing file already imports from `jarvis.chunker` at line 7 — add `skip_reason` to that import list.
...
Expected: FAIL — `ImportError: cannot import name 'skip_reason' from 'jarvis.chunker'`

- [x] **Step 3: Add the constants**

In `src/jarvis/chunker.py`, immediately after `MAX_IMPORT_LINES = 10` (line 28):
...
In `src/jarvis/chunker.py`, immediately after `language_for()` (which ends at line 77):
...
In `tests/test_chunker.py`, line 7, add `skip_reason` to the existing import from `jarvis.chunker`.
...
**Files:**
- Modify: `src/jarvis/embeddings.py` (add method to `EmbeddingModel` after `embed_texts()`, which ends at line 64)
...
In `src/jarvis/embeddings.py`, add to `EmbeddingModel` right after `embed_texts()` (ends line 64), before `embed_query()`:
...
**Files:**
- Modify: `src/jarvis/semantic.py` (import line 15; add dataclasses after `FusedHit` which ends line 38; rewrite `index_semantic()` at lines 150-191)
...
In `src/jarvis/semantic.py`, line 15, add `skip_reason` to the existing chunker import:
...
In `src/jarvis/semantic.py`, after the `FusedHit` dataclass (ends line 38):
...
**Files:**
- Modify: `src/jarvis/registry.py` (schema line 26; new migration fn after line 65; `RegisteredRepo` line 68-77; `_row_to_repo` line 80-91; `Registry.__init__` line 106; `upsert` line 108-132; `get` line 150; `list` line 158)
...
In `src/jarvis/registry.py`, change `_SCHEMA` (lines 18-29) so the `repos` table ends with the new column:
...
In `src/jarvis/registry.py`, after `_ensure_semantic_indexed_at_column` (ends line 65):
...
def _ensure_semantic_include_column(conn: sqlite3.Connection) -> None:
    ...
    `jarvis watch` reindex) is re-raised rather than swallowed."""
    try:
...
In `src/jarvis/registry.py`, add above `RegisteredRepo` (line 68):
...
**Files:**
- Modify: `src/jarvis/index_cli.py` (`_run_semantic_stage` lines 220-245; `index_repo` signature line 248-250 and body lines 270-276, 313, 325; `_cmd_index` lines 344-351; `_cmd_reindex` line 399; parser line 522-526)
...
In `src/jarvis/index_cli.py`, immediately after `_resolve_scheme` (ends line 217):
...
In `src/jarvis/index_cli.py`, after the existing `index_parser` arguments (the `--scheme` block ending at line 526):
...
uv run python -c "
from pathlib import Path
from jarvis.chunker import chunk_file, hash_file, iter_source_files, language_for, skip_reason
