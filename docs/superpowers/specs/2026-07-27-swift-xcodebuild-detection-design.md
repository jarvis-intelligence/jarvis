...
`codeintel index` fails on Swift repos that are UIKit-only SPM packages (`platforms: [.iOS(...)]`,
...
Fix is scoped to `codeintel` only — `scip-swift` is a separate personal project and is not
modified by this design.

## Scope

In scope: `codeintel`'s Swift indexing path (`index_cli.py`, `registry.py`, the `index`/`reindex`/
...
- `codeintel index <path> --scheme <name>` (new optional CLI flag on the `index` subparser) sets
  it on first index.
- `codeintel reindex <slug>` reads `repo.scheme_override` back out of the registry and passes it
  through to `index_repo()` — no need to repeat `--scheme` on every reindex.
- `codeintel watch <path> --scheme <name>` (same new flag on the `watch` subparser) threads it
...
- If `xcodebuild` is selected and no scheme is set (and `scip-swift` can't auto-resolve because
  ...
  extra codeintel-level wrapping needed.
...
```bash
codeintel index /Users/ddphuong/Projects/epost-workspace/epost-app/epost-ios-theme-showcase/epost-ios-theme-ui \
