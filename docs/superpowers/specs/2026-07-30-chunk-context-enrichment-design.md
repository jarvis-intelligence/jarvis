...
unlike earlier BGE versions — but `JARVIS_EMBEDDING_MODEL` can select a model
...
Environment variables: `JARVIS_EMBEDDING_QUERY_PREFIX`,
`JARVIS_EMBEDDING_DOC_PREFIX`. Each is independent — setting only one is
...
4. `jarvis index` on this repo reports a `chunk tokens p50=… p90=… max=…`
   line.
