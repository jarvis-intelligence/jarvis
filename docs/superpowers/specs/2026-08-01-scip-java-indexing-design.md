...
`codeintel index` fails on every Java/Kotlin repo with
...
upstream change, not something codeintel can work around.
...
Three distinct failure modes, only two of which codeintel can act on:
...
Decoded with codeintel's own `scip_pb2` against a fixture with a real hierarchy:
...
Repos that cannot get SCIP navigation currently get *nothing* from codeintel: `index_repo()` runs
...
- `.claude/skills/codeintel-setup/SKILL.md`: `scip-java (detect-only)` becomes a real install.
...
compile tasks *are* `JavaCompile` tasks. A codeintel-owned Gradle init script plus
...
- Sentinel value for `registry.language` when no SCIP-indexable language is found (`"none"`,
  `"unknown"`, `"-"`). Cosmetic, but it surfaces in `codeintel list` / `status`.
