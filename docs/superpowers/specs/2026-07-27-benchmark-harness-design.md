# Benchmark harness (`codeintel bench`) — Design

## Problem

`codeintel` has no way to measure how fast indexing or querying actually is, and no way to
...
In scope: a new `codeintel bench` CLI subcommand, a new `src/codeintel/bench.py` module
(storage, comparison, drift-check, output formatting), a `~/.codeintel/bench.db` history store,
...
`codeintel bench` is a new argparse subcommand in `index_cli.py`, alongside `index`/`list`/
...
5. Writes every measurement into `~/.codeintel/bench.db` (Section 2), then prints a comparison
...
CREATE TABLE runs (
    ...
    codeintel_commit TEXT NOT NULL  -- git rev-parse HEAD of codeintel itself
);
...
One `runs` row per `codeintel bench` invocation, recording which version of `codeintel` produced
...
New module `src/codeintel/bench.py` holds the actual logic (storage, comparison/regression
...
which ships inside the repo). `codeintel bench` itself, run by hand, is that verification.

## Follow-ups (not part of this design)

- Wiring `codeintel bench` into CI once a CI test workflow exists at all (currently: no workflow
  runs pytest either).
