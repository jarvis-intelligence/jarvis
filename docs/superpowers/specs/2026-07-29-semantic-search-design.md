...
Add semantic (natural-language) code search to jarvis, fused with the existing
...
| Pipeline | Third stage inside existing `jarvis index` | One command, one registry status; no separate embed step |
...
- Env vars: `JARVIS_EMBEDDING_MODEL` (default `BAAI/bge-m3`),
  `JARVIS_EMBEDDING_BATCH_SIZE` (default 32).

## Storage & Search (`semantic.py`)

**Layout:** `~/.jarvis/lancedb/` holds one table per repo, named by slug. This
...
**Write path (during `jarvis index`):** hash each file in the current file
...
- The semantic stage is optional and non-fatal: without the `semantic` extra,
  `jarvis index` logs one skip line and continues — zero behavior change for
  existing users.
...
- Registry gains one nullable column `semantic_indexed_at`, surfaced by
  `jarvis status`. `reindex` and `watch` run the semantic stage automatically
...
| LanceDB table missing at query time | `{"error": "no semantic index for <slug> — run jarvis reindex <slug>"}` |
