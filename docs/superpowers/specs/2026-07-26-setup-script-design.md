...
jarvis requires 7 external binaries on `PATH` (one SCIP indexer per language, `scip` for
...
is enough to go from a bare macOS or Linux machine to a fully-working `jarvis` install.
...
| `zoekt-index` / `zoekt-webserver` | Download from jarvis's own GitHub release assets — see "New CI: zoekt binaries" below |
...
4. All downloaded binaries land in a dedicated `~/.jarvis/bin`. If that directory isn't
...
without ctags present; search works, symbol-aware ranking is reduced. jarvis's
...
- Publish the built binaries as assets on jarvis's own GitHub Releases.
- `setup.sh`'s zoekt installer downloads from jarvis's release assets, not upstream.
...
v0.9.0 has not yet been *executed* against jarvis — only its schema proven identical —
...
- A `jarvis setup`/`doctor` CLI subcommand — this is a standalone shell script for now; folding
