...
Give jarvis a single-page landing page that lets a skeptical developer
...
- Reuses the existing diagrams already in `docs/assets/`:
  `jarvis-architecture.png`, `jarvis-system-architecture.png`.
...
3. **How it works** — the **three verified SVG diagrams** committed in
   ...
   - `jarvis-core-components.svg` — pipeline, embedding, storage
   - `jarvis-query-engine.svg` — SCIP path vs. hybrid path, no router
   - `jarvis-agent-integration.svg` — MCP over stdio, 9 flat tools
...
5. **Requirements & limits** — a compact, honest version of the README's
   ...
   jarvis never edits code; **`typeHierarchy` is non-functional** pending
...
6. **Install** — the CTA: `setup.sh` → `uv tool install jarvis-mcp`
   → `jarvis index`, plus the `/plugin marketplace add phuongddx/jarvis`
...
`src/jarvis/` and each visually verified after export:

| File | Covers |
|---|---|
| `jarvis-core-components.svg` (43KB) | 6-stage pipeline, embedding generation, 4 storage stores |
| `jarvis-query-engine.svg` (28KB) | SCIP structural path vs. hybrid vector+lexical path, RRF k=60 |
| `jarvis-agent-integration.svg` (27KB) | stdio JSON-RPC, 9 flat tools, response contract |
...
`jarvis-architecture.png` / `jarvis-system-architecture.png` is a
...
**`jarvis-architecture.excalidraw`**
...
**`jarvis-system-architecture.excalidraw`**
| Element | Says | Must say |
|---|---|---|
