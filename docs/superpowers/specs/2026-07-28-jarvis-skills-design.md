# jarvis Skill Toolkit — Design

## Purpose

jarvis is a local-first code-intelligence MCP server (SCIP navigation + Zoekt search, 8 MCP
tools). The README documents it well, but an agent working *with* jarvis needs three things the
README doesn't give it: (1) a linear onboarding runbook, (2) an everyday decision matrix that makes
the agent **prefer jarvis's structural tools over grep**, and (3) a feedback loop for filing
jarvis bugs/features. This spec defines three skills that cover that lifecycle:
...
**Build:** three `jarvis-*` skills under `.claude/skills/` in this repo, each a single
...
**Out of scope:**
- Eval harness (above).
- Auto-installing `gh` / `gh auth` checks inside `jarvis-issues` — assumes `gh` installed and
  authed.
- Papering over jarvis's own limitations (one-language-per-repo, single-tenant hardcoding) —
  the skills document them honestly, they don't hide them.
- Windows — jarvis itself doesn't support it.

## Architecture

```
jarvis/                                   # repo root = CWD
├── .claude/skills/
│   ├── jarvis-setup/
...
│   └── jarvis-issues/
│       └── SKILL.md                         # file jarvis bugs via gh
├── scripts/
...
3. **`references/`** — loaded as-needed. `jarvis-use` spills the 8-tool detail to
...
|---|---|
| Location `.claude/skills/` in CWD | Skills under `.claude/skills/jarvis-*/` |
...
| SKILL.md <300 lines | Enforced; `jarvis-use` splits detail to `references/` |
...
### Skill 1 — `jarvis-setup`

**Intent:** get a new machine/repo from zero to "jarvis indexed and answering queries" in one
...
**Body sections:**
1. **Prerequisites check** — macOS/Linux, `uv` present, `~/.jarvis/bin` on `PATH`.
2. **Install** — `curl -fsSL https://raw.githubusercontent.com/phuongddx/jarvis-dist/main/setup.sh | sh`
   then `uv sync`. One command each, idempotent.
3. **Register MCP** — the exact `claude mcp add jarvis --scope user -- uv --directory
   <path> run jarvis-server` line; one-line note for Cursor/other clients.
4. **Index a repo** — `jarvis index /path [--slug name] [--scheme name]`, with the
   one-language-per-repo detection rule.
5. **Verify** — `jarvis status <slug>` then a smoke `goToDefinition` call; expected output
   shape.
...
7. **Next** — one-line pointer to `jarvis-use`.

**Does NOT cover:** everyday queries (→ `jarvis-use`), issue filing (→ `jarvis-issues`).

### Skill 2 — `jarvis-use`

**Intent:** make the agent **prefer jarvis's MCP tools for structural queries**, check freshness
...
- For any *structural* query (definition, references, callers, hierarchy): call `getIndexStatus`
  first.
  - `indexed: true`, fresh → use the jarvis tool.
  - `indexed: true`, stale → run `jarvis reindex <slug>`, then use the tool.
...
- `typeHierarchy` returns an explicit error on real indexes — upstream `scip expt-convert` never
  populates `relationships`, not a jarvis bug.
...
moves to `references/tool-roster.md`. SKILL.md carries the matrix + the prefer-jarvis rule + a
grep-pointer (`grep -n "tool-roster"`) to that file.

### Skill 3 — `jarvis-issues`

**Intent:** file well-formed bug reports and feature requests against the jarvis project on
GitHub via `gh`.
...
**Body sections:**
1. **Gather context first** — exact command run, repo + slug, `jarvis status <slug>` output, tool
   name + arguments, full error payload, jarvis version (`uv run jarvis --version`), OS/arch.
...
4. **File via `gh`** — `gh issue create --repo phuongddx/jarvis --title "..." --body "..."`.
...
**Does NOT cover:** general GitHub issues for other repos — only the jarvis project itself.

### Shared conventions

- **Cross-link:** each SKILL.md opens with one line — *"Part of the jarvis toolkit. Siblings:
  `jarvis-setup` (onboard), `jarvis-use` (everyday), `jarvis-issues` (report)."* Pointer
...
`.claude/skills/jarvis-*`, create `~/.zcode/skills/<name>` → repo path if absent, skip if
...
- Skills are documentation, not executable logic — the only executable is `link_skills.py`, whose
  ...
  exit non-zero.
- `jarvis-issues` instructs the agent to confirm with the user *before* `gh issue create` —
...
- The skills never instruct the agent to mutate a published `index-<sha>.db` or otherwise violate
  jarvis's read-only-runtime / atomic-publish invariants.
