...
Measured against jarvis's own `src/` tree:
...
semantic: skipped src/jarvis/scip_pb2.py (banner:do not edit)
...
Additionally: `jarvis index` on this repo must name `scip_pb2.py` in its
skip report, and a second `jarvis reindex` must leave no `scip_pb2.py` rows
...
| Model-aware embedding prefixes | Latent, not active: `embed_query` applies no prefix regardless of model, so swapping `JARVIS_EMBEDDING_MODEL` to nomic/e5 would silently degrade retrieval. The bge-m3 default is correct today. Separate module, separate spec. |
