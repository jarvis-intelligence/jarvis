...
phuongddx/jarvis` + `/plugin install jarvis@jarvis`) up into Quick
Start's step 4, replacing the raw `claude mcp add jarvis --scope user --
jarvis-server` line as the lead path. Previously this install method only
...
wires up `uvx --from jarvis-mcp jarvis-server` as an MCP
server entry; there is no MCP tool for indexing anywhere in the tool roster.
`jarvis index` is CLI-only, full stop. If that draft had gone out, someone
...
- `plugin/.mcp.json` registers exactly one MCP server command:
  `uvx --from jarvis-mcp jarvis-server`. No `index` tool
