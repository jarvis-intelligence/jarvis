...
**jarvis** is a personal, local-first code intelligence MCP server that brings structural code navigation (SCIP-backed), lexical search (Zoekt-backed), and natural-language semantic/vector search to Claude Code, Cursor, and any MCP client. It runs as a single stdio process — no server, no auth, no network.
...
- `watchdog` (optional, `--extra watch`) — file monitor for `jarvis watch`
...
- `~/.jarvis/lancedb/` — one LanceDB vector table per repo (semantic search), gated behind the `semantic` extra
...
- `jarvis index <repo>` end-to-end: detect language → run language indexer → `scip expt-convert` → zoekt-index → atomic pointer swap → registry update ✓
