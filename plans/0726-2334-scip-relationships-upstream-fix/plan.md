...
- Produces: an accurate cross-reference in the codeintel plan

- [ ] **Step 1: Link the PR from the codeintel plan**

The codeintel plan's Task 7 and Task 8 currently cite issue #464 as the upstream status. Add the PR alongside it so a future reader knows a fix is in flight:
...
git commit -m "docs: link the upstream relationships PR from the codeintel plan"
```
