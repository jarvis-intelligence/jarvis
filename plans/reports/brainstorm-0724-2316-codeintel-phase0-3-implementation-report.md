# Brainstorm Report — codeintel Phase 0–3 Implementation
...
| `config.py` | new | data_dir (~/.codeintel), zoekt port/paths, env overrides |
...
Storage: `~/.codeintel/repos/{project}/{repo}/index-{sha}.db` + `current` pointer; `registry.db`; `.zoekt/` shards.
