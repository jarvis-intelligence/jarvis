...
## Scope
- `src/codeintel/graph.py` (new), `src/codeintel/watch.py` (new), `src/codeintel/index_cli.py` (modified), `src/codeintel/server.py` (modified), `tests/test_graph.py`, `tests/test_watch.py`, `tests/test_server_tools.py`
...
`src/codeintel/graph.py:260-292`. Every call only *adds* rows (`upsert_package`, `add_edge`); there is no delete/reconcile step. Concretely:
...
- If a local package is renamed (or a repo is `codeintel forget`+re-added under the same slug with a different package name), the old `packages` row for the old name is orphaned permanently and stays traversable.
...
**2. Unhandled exception classes can silently kill the `codeintel watch` background loop.**
`src/codeintel/index_cli.py:265-294`. `_reindex()` only catches `(UnsupportedLanguageError, IndexingError, ValueError)`. But several calls inside `index_repo()` execute **before** its own `try:` block (line 118) and are therefore never converted to `IndexingError`:
...
`plans/0724-2316-codeintel-mcp-implementation/phase-04-graph-blast-radius-and-auto-reindex.md` success criteria:
