# Code Review: codeintel Phase 2 + Phase 3 (server, query, index_cli, registry, search)

## Scope
- Files reviewed: `src/codeintel/{models,config,query,server,registry,search,index_cli,index_reader}.py`, `pyproject.toml`, `README.md`, tests.
- Tests: `uv run pytest -q` → **63 passed**, no failures, no sandbox hangs observed.
- Plan cross-check: `plans/0724-2316-codeintel-mcp-implementation/{plan,phase-02,phase-03}.md` — all listed success criteria appear met (7 tools registered, CRUD registry, atomic swap, lazy zoekt lifecycle). Marked "Completed" in plan.md; consistent with what's in the tree.
...
Because `.` and `-` pass the allow-list regex unchanged, a value like `--slug ..` or `codeintel forget ..` produces a path component of `..`, letting `index_dir()` walk out of `~/.codeintel/scip/_/`. Combined with `forget`'s unconditional `shutil.rmtree`, a typo'd or copy-pasted slug (e.g. `codeintel forget ../../Documents`) can **recursively delete a directory outside codeintel's data dir**. `index` has the same issue for writes (arbitrary file creation via `shutil.copy`/pointer writes).
...
- Do you want `codeintel forget`/`reindex`/`status` to also validate `args.slug` against the registry *before* touching any path (i.e., reject slugs that were never registered) as a second layer of defense, or is the `repo_slug()` sanitization alone sufficient given this is a personal tool?
