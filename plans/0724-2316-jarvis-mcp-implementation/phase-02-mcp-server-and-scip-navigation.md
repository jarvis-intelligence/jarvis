...
Port the query layer (5 SCIP nav tools + getIndexStatus) and wrap it in an MCP stdio server. Ends with jarvis registered user-scope in Claude Code and hand-verified answers on a TS validation repo (TS) + the source project (Python).
...
- `config.py`: `data_dir` (default `~/.jarvis`, env `JARVIS_DATA_DIR`), repo→index path resolution `repos/{repo_slug}/index-{sha}.db` + `current` pointer file (one line: filename).
...
- Interim indexing for acceptance (CLI arrives Phase 3): document exact manual commands in README:
  `scip-typescript index` / `scip-python index .` → `scip expt-convert --format sqlite index.scip` → place as `~/.jarvis/repos/<slug>/index-<sha>.db` + write `current`.

## Related Code Files

- Create: `src/jarvis/query.py` (port), `src/jarvis/config.py`, `src/jarvis/server.py`
...
1. Port `tests/test_query.py` from `test_api_navigation.py` — target the NEW module API (`jarvis.query`). **Red.**
...
5. Acceptance (manual, documented in README):
   ...
   - `claude mcp add jarvis --scope user -- uv --directory ~/Projects/jarvis run jarvis-server`
