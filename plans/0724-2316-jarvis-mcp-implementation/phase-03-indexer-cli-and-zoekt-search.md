...
Replace the manual index recipe with `jarvis` CLI (full pipeline incl. zoekt-index + atomic swap + registry), and add searchCode backed by an embedded zoekt-webserver.

## Requirements

- Functional: `jarvis index|list|status|reindex|forget`; searchCode via MCP returns Zoekt hits scoped to a repo.
...
- `index_cli.py` (argparse, entry point `jarvis`): detect language by extension scan (`.ts/.tsx`→scip-typescript, `.py`→scip-python, `.rs`→rust-analyzer, `.java/.kt`→scip-java) → run indexer binary → `scip expt-convert` → `zoekt-index -index ~/.jarvis/.zoekt <repo>` → write `index-{sha}.db`, flip `current` pointer, delete old, update registry. Assert `scip` CLI convert schema v0.7.0 (`scip version` check).
- `search.py`: port `$SOURCE_REPO/.../service/search_service.py` (225 LOC — httpx client to zoekt-webserver JSON API `{"Q": ...}`, verified 2026-07-12). Add lifecycle mgr: lazy-spawn `zoekt-webserver -index ~/.jarvis/.zoekt -listen :PORT` on first searchCode, pidfile in data_dir, health check, atexit kill.
- `server.py`: register searchCode (tool #7).

## Related Code Files

- Create: `src/jarvis/registry.py`, `src/jarvis/index_cli.py`, `src/jarvis/search.py`
...
- Modify: `src/jarvis/server.py`, `pyproject.toml` (`[project.scripts] jarvis = ...`), `README.md`
...
5. Acceptance: `jarvis index` on a TS validation repo + the source project; `jarvis list/status` sane; searchCode from Claude Code returns expected hits; `jarvis reindex` while querying in parallel → no errors (atomic swap proof); no zoekt orphan after server exit (`pgrep zoekt-webserver`).
...
- Binary paths differ per machine → config.py resolves from PATH with env overrides (`JARVIS_ZOEKT_BIN` etc.).
