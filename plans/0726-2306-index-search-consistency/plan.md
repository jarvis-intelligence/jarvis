...
|---|---|
| `src/codeintel/scip_pb2.py` (regenerate) | Vendored gencode, moved from scip.proto v0.7.0 → v0.9.0 so `typed_range` exists |
| `src/codeintel/scip_decoder.py` (modify) | Read `typed_range` with deprecated-`range` fallback (no change to `ScipOccurrence`'s shape) |
| `src/codeintel/index_cli.py` (modify) | `scip` version floor; `-meta` slug naming for zoekt; shard teardown on `forget`; post-convert ingestion gate |
| `src/codeintel/query.py` (modify) | Capability probes (`occurrence_data_present`, `relationship_data_present`); `type_hierarchy` reports unavailability |
| `src/codeintel/server.py` (modify) | Surface `typeHierarchy` unavailability as an explicit payload |
...
**Files:**
- Regenerate: `src/codeintel/scip_pb2.py`
...
head -3 scip_pb2.py
cp scip_pb2.py /Users/ddphuong/Projects/codeintel/src/codeintel/scip_pb2.py
```

Then restore the vendoring header. Open `src/codeintel/scip_pb2.py` and insert this immediately after the first `# -*- coding: utf-8 -*-` line, replacing any header the previous version had:
...
```bash
git add src/codeintel/scip_pb2.py pyproject.toml uv.lock tests/test_scip_decoder.py
...
**Files:**
- Modify: `src/codeintel/scip_decoder.py`
- Test: `tests/test_scip_decoder.py`
...
CODEINTEL_SETUP_SOURCED=1 CODEINTEL_BIN_DIR=/tmp/swiftproof/bin FORCE=1 \
...
PATH="/tmp/swiftproof/bin:$PATH" CODEINTEL_DATA_DIR=/tmp/swiftproof/data \
  uv run codeintel index /tmp/swiftproof/repo --slug swiftproof
...
```bash
CODEINTEL_DATA_DIR=/tmp/swiftproof/data uv run python -c "
...
```bash
git add src/codeintel/scip_decoder.py tests/test_scip_decoder.py
...
In `src/codeintel/index_cli.py`, add near the other module constants (after `_IGNORED_DIRS`):
...
```bash
uv run codeintel index /tmp/swiftproof/repo --slug floor-check 2>&1 | tail -3
```
...
```bash
PATH="/tmp/swiftproof/bin:$PATH" CODEINTEL_DATA_DIR=/tmp/swiftproof/data \
  uv run codeintel index /tmp/swiftproof/repo --slug floor-check 2>&1 | tail -2
```
...
In `src/codeintel/index_cli.py`, add above `index_repo`:
...
cd /Users/ddphuong/Projects/codeintel
PATH="/tmp/swiftproof/bin:$PATH" CODEINTEL_DATA_DIR=/tmp/searchproof/data \
  uv run codeintel index /tmp/searchproof/somedirname --slug renamed-slug
CODEINTEL_DATA_DIR=/tmp/searchproof/data uv run python -c "
from codeintel import config
...
Confirmed defect: `_cmd_forget` (`index_cli.py:231-249`) deletes the registry row and the SCIP index directory but never the shard. Observed live — the registry is empty while `~/.codeintel/.zoekt/` still holds `mini_py_repo`, `mini_swift_repo`, and `scip-swift` shards, so `searchCode` returns hits for repos codeintel no longer knows about, pointing at paths that may no longer exist.
...
In `src/codeintel/index_cli.py`, add above `_cmd_forget`:
...
```bash
uv run codeintel list          # expect no output (empty registry)
ls ~/.codeintel/.zoekt/        # expect the 3 orphans
rm -f ~/.codeintel/.zoekt/*.zoekt
ls ~/.codeintel/.zoekt/        # expect empty
```
If `codeintel list` is NOT empty, do not delete blindly — reindex those repos after Task 4 instead, so their shards are recreated under the slug name.
...
searchCode kept returning hits for repos codeintel no longer knew about."
```
...
This is the structural guard: it is exactly the fingerprint that cost real debugging time this session (`documents=1, global_symbols=7, chunks=0, mentions=0`). Publishing still happens — the symbol table is genuinely useful — but the status stops claiming full success, so `codeintel list`/`status` shows it.
...
In `src/codeintel/index_cli.py`, add the constant beside `MIN_SCIP_VERSION`:
...
```bash
PATH="/tmp/swiftproof/bin:$PATH" CODEINTEL_DATA_DIR=/tmp/swiftproof/data \
  uv run codeintel index /tmp/swiftproof/repo --slug gatecheck
CODEINTEL_DATA_DIR=/tmp/swiftproof/data uv run codeintel status gatecheck
```
...
**Files:**
- Modify: `src/codeintel/query.py`, `src/codeintel/server.py`
...
In `src/codeintel/query.py`, add above `class QueryService`:
...
In `src/codeintel/server.py`, update the tool. Replace the body of `type_hierarchy`:
...
```bash
CODEINTEL_DATA_DIR=/tmp/swiftproof/data uv run python -c "
import codeintel.server as server
...
```bash
git add src/codeintel/query.py src/codeintel/server.py tests/test_query.py tests/test_server_tools.py
...
The roadmap currently states the Swift gap is that "`scip-swift`'s emitted occurrences carry no `Range` data — a gap in `scip-swift` itself." **That diagnosis is wrong** and must be corrected: scip-swift emits `single_line_range` correctly; the ranges were lost because `scip v0.7.0` cannot read the `typed_range` oneof and codeintel's vendored gencode had no such field.
...
single_line_range, and both scip v0.7.0 and codeintel's v0.7.0-era gencode
